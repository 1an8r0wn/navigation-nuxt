...
<template>
  <nav class="w-full sm:w-52 sm:min-w-52 h-auto sm:h-dvh sticky top-0 z-50 pr-0 sm:pr-4 mb-4 sm:mb-0 flex flex-row sm:flex-col overflow-x-auto">
    <div
    ...
    >
      ...
      <div class="max-sm:h-12 flex items-center flex-nowrap bg-white dark:bg-transparent">
        <span
          :class="{ 'bg-zinc-100 text-zinc-800 font-bold dark:text-zinc-300 dark:bg-zinc-900/30 backdrop-blur-md': category.id === activeIndex }"
          class="visible sm:hidden w-max mx-4 py-2 px-4 text-zinc-400 active:text-zinc-800 active:font-bold dark:text-zinc-600 dark:active:text-zinc-300 backdrop-blur-sm bg-transparent active:bg-zinc-100 dark:active:bg-zinc-900 rounded-full cursor-pointer"
        >
          {{ category.name }}
        </span>
      </div>
